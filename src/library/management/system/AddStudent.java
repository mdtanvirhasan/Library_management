...
public class AddStudent extends JFrame implements ActionListener{
   ...
   /* public void random() {
	...
	comboBox.setModel(new DefaultComboBoxModel(new String[] { "Mechanical", "CSE",  "Civil",  "EEE", "Other" }));
...
//	random();
     ...
     /*public void autoid()
    {
        try {
            ...
            if (rs.next()) 
            {
               int rnno =rs.getInt("Student_id");
                //int co = rnno.length();
                Random r= new Random();
                HashSet<Integer> set = new HashSet<>();
                while (set.size()<1) 
                {
                int random =r.nextInt(9999)+10;
               int snum =random;
                int  ftxt = snum;
                 t1.setText(ftxt);
    ...
    public void actionPerformed(ActionEvent ae){
        try{
           ...
           if(ae.getSource() == b1){
                ...
                conn con = new conn();
                String sql = "insert into student( name, father, course, branch, year, semester) values( ?, ?, ?, ?, ?, ?)";
		PreparedStatement st = con.c.prepareStatement(sql);
